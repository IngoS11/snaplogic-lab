...
resource "google_compute_subnetwork" "sap_4h_us_west1" {
  name                     = "sap-a4h-${var.region}-subnet"
  ip_cidr_range            = "192.168.1.0/24"
  region                   = var.region
  network                  = google_compute_network.sap_4h_vpc.self_link
...
resource "google_compute_instance" "sap_a4h_dev" {
  ...
  network_interface {
    subnetwork = google_compute_subnetwork.sap_4h_us_west1.self_link
